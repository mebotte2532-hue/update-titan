...
import re, sys, os, requests, tempfile
...
MONSHI2_STATE_FILE = "monshi2.json"
...
_feature_state = _load_json_file(FEATURE_STATE_FILE, {
    "enemy": [], "love": [], "pvlock_users": []
})
enemy = sorted(set(int(x) for x in _feature_state.get("enemy", []) if str(x).lstrip("-").isdigit()))
love = sorted(set(int(x) for x in _feature_state.get("love", []) if str(x).lstrip("-").isdigit()))
pvlock_users = set(int(x) for x in _feature_state.get("pvlock_users", []) if str(x).lstrip("-").isdigit())
spam_tasks = {}

def _save_feature_state():
    _save_json_file(FEATURE_STATE_FILE, {
        "enemy": enemy,
        "love": love,
        "pvlock_users": sorted(pvlock_users)
    })

def _target_user_id(app, message):
    """Resolve a Telegram user from reply, numeric ID or username."""
    if message.reply_to_message and message.reply_to_message.from_user:
        return int(message.reply_to_message.from_user.id)
    parts = (message.text or "").split()
    if len(parts) < 2:
        raise ValueError("کاربر را با ریپلای، یوزرنیم یا آیدی عددی مشخص کنید.")
    raw = parts[1].strip()
    if raw.lstrip("-").isdigit():
        return int(raw)
    user = app.get_users(raw if raw.startswith("@") else f"@{raw}")
    return int(user.id)

def _user_label(app, user_id):
    try:
        u = app.get_users(user_id)
        name = " ".join(x for x in [u.first_name, u.last_name] if x) or str(user_id)
        return f'<a href="tg://user?id={user_id}">{html.escape(name)}</a>'
    except Exception:
        return f'<a href="tg://user?id={user_id}">کاربر</a>'

# ---------------------- MajidAPI integration ----------------------
MAJID_API_TOKEN = os.getenv("MAJID_API_TOKEN", "").strip()
MAJID_API_BASE = os.getenv("MAJID_API_BASE", "https://api.majidapi.ir").rstrip("/")

def _majid_headers():
    return {"Accept": "application/json", "User-Agent": "TiTaN-Self/3.0"}

def _majid_json(path, params=None, timeout=45):
    """Synchronous HTTP wrapper; call through asyncio.to_thread from async handlers."""
    params = dict(params or {})
    if MAJID_API_TOKEN:
        params.setdefault("token", MAJID_API_TOKEN)
    url = f"{MAJID_API_BASE}/{path.lstrip('/')}"
    r = requests.get(url, params=params, headers=_majid_headers(), timeout=timeout)
    r.raise_for_status()
    try:
        data = r.json()
    except ValueError:
        raise RuntimeError(f"MajidAPI پاسخ JSON معتبر برنگرداند (HTTP {r.status_code}).")
    if isinstance(data, dict):
        # Common error shapes used by APIs.
        if data.get("success") is False or data.get("ok") is False:
            raise RuntimeError(str(data.get("message") or data.get("error") or "MajidAPI request failed"))
...
def _walk_values(obj, keys=()):
    if isinstance(obj, dict):
        for k, v in obj.items():
            yield str(k).lower(), v
            yield from _walk_values(v, keys + (str(k).lower(),))
...
def _collect_urls(obj):
    urls = []
    url_keys = {"url", "download", "downloadurl", "download_url", "media", "src",
                "video", "video_url", "image", "image_url", "thumbnail", "profile",
                "profile_pic", "profile_photo", "hd", "high"}
    def visit(x):
        if isinstance(x, dict):
            for k, v in x.items():
                kl = str(k).lower().replace("-", "_")
                if isinstance(v, str) and v.startswith(("http://", "https://")) and (
                    kl in url_keys or any(part in kl for part in ("url", "download", "media"))
                ):
                    urls.append(v)
                else:
                    visit(v)
...
async def _majid_request(path, params=None, timeout=45):
    if not MAJID_API_TOKEN:
        raise RuntimeError("MAJID_API_TOKEN در Environment Variables تنظیم نشده است.")
    return await asyncio.to_thread(_majid_json, path, params, timeout)
...
def _instagram_url(url):
    return bool(re.match(r"^https?://(?:www\.)?(?:instagram\.com|instagr\.am)/", url, re.I))

def _extract_instagram_profile(data, username):
    return {
        "username": _first_value(data, ["username", "user_name", "handle"], username),
        "name": _first_value(data, ["full_name", "fullname", "name"], "—"),
        "followers": _first_value(data, ["followers", "follower_count", "followers_count"], "—"),
        "following": _first_value(data, ["following", "following_count"], "—"),
        "posts": _first_value(data, ["posts", "post_count", "media_count"], "—"),
        "bio": _first_value(data, ["bio", "biography", "description"], "—"),
        "verified": _first_value(data, ["verified", "is_verified"], False),
        "private": _first_value(data, ["private", "is_private"], False),
        "photo": _first_value(data, ["profile", "profile_photo", "profile_pic", "profile_picture",
                                     "profile_picture_url", "avatar", "avatar_url"], None),
    }

def _profile_photo_url(data):
    value = _first_value(data, ["profile", "profile_photo", "profile_pic", "profile_picture",
                                 "profile_picture_url", "avatar", "avatar_url"], None)
    if isinstance(value, dict):
        return _first_value(value, ["url", "src", "hd", "high", "original"], None)
    return value if isinstance(value, str) and value.startswith(("http://", "https://")) else None

def _is_media_url(value):
    if not isinstance(value, str) or not value.startswith(("http://", "https://")):
        return False
    lower = value.lower().split("?", 1)[0]
    return lower.endswith((".jpg", ".jpeg", ".png", ".webp", ".gif", ".mp4", ".mov", ".m4v", ".webm"))


async def _download_remote_file(url):
    suffix = os.path.splitext(url.split("?", 1)[0])[1][:8]
    fd, path = tempfile.mkstemp(prefix="titan_media_", suffix=suffix)
    os.close(fd)
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers={"User-Agent": "Mozilla/5.0"}) as session:
            async with session.get(url, allow_redirects=True) as resp:
                if resp.status >= 400:
                    raise RuntimeError(f"Media server returned HTTP {resp.status}")
                ...
                with open(path, "wb") as f:
                    async for chunk in resp.content.iter_chunked(1024 * 256):
                        ...
                        if size > 200 * 1024 * 1024:
                            raise RuntimeError("فایل بزرگ‌تر از سقف 200MB است.")
                        f.write(chunk)
...
async def _send_majid_media(app, chat_id, url, caption=None, reply_to=None):
    path = None
    try:
        path, content_type = await _download_remote_file(url)
        lower = (content_type or "").lower()
        ext = os.path.splitext(path)[1].lower()
        kwargs = {"chat_id": chat_id, "reply_to_message_id": reply_to}
        if caption:
            kwargs["caption"] = caption
        if lower.startswith("image/") or ext in {".jpg", ".jpeg", ".png", ".webp"}:
            await app.send_photo(photo=path, **kwargs)
        elif lower.startswith("video/") or ext in {".mp4", ".m4v", ".mov", ".webm"}:
            await app.send_video(video=path, **kwargs)
        elif lower.startswith("audio/") or ext in {".mp3", ".m4a", ".ogg", ".wav"}:
            await app.send_audio(audio=path, **kwargs)
        else:
            await app.send_document(document=path, **kwargs)
    finally:
...
async def _majid_download_media(app, chat_id, source_url, reply_to=None, status_message=None):
    if not MAJID_API_TOKEN:
        raise RuntimeError("MAJID_API_TOKEN در Railway Variables تنظیم نشده است.")
    # The generic endpoint is kept first because the user's current token page
    # exposes it. Instagram has an officially documented dedicated endpoint,
    # so it is used as a safe fallback.
    try:
        data = await _majid_request("social/download", {"url": source_url}, timeout=60)
    except Exception as generic_error:
        if not _instagram_url(source_url):
            raise generic_error
        data = await _majid_request("instagram/download", {"url": source_url, "out": "url"}, timeout=60)

    urls = _collect_urls(data)
    # Prefer actual media URLs over thumbnails/profile links.
    media = [u for u in urls if _is_media_url(u)]
    if media:
        urls = media
    if not urls:
        raise RuntimeError("MajidAPI لینک دانلود معتبری در پاسخ برنگرداند.")

    # De-duplicate and cap a single post to Telegram's practical media-group size.
    urls = list(dict.fromkeys(urls))[:10]
    for index, media_url in enumerate(urls):
        if status_message and index == 0:
            try:
                await status_message.edit_text(f"⬇️ در حال دریافت محتوا...\\n📦 {len(urls)} فایل شناسایی شد")
            except Exception:
                pass
        await _send_majid_media(
            app, chat_id, media_url,
            caption=("TiTaN Self Downloader" if index == 0 else None),
            reply_to=reply_to
        )
    return len(urls)
...
async def actions1(app, message):
    """Private-message guards: per-user PV lock + monshi2 mandatory membership."""
    if not message or not message.from_user or int(message.from_user.id) == int(admin):
        return
    if not getattr(message, "text", None) and not getattr(message, "media", None):
        return

    if int(message.from_user.id) in pvlock_users:
        try:
    ...
    cfg = _load_json_file(MONSHI2_STATE_FILE, {
        "enabled": False, "text": "برای ادامه لطفاً در موارد زیر عضو شوید:",
        "photo_file_id": None, "chats": []
    })
    if not cfg.get("enabled") or not cfg.get("chats"):
        return

    user_id = int(message.from_user.id)
    missing = []
    for item in cfg.get("chats", []):
        try:
            member = await app.get_chat_member(int(item["id"]), user_id)
        except Exception:
            missing.append(item)
            continue
        status = str(getattr(member, "status", "")).lower()
        is_member = status in {"member", "administrator", "owner", "creator"} or (
    ...
    for item in cfg.get("chats", []):
        if item.get("link"):
            rows.append([InlineKeyboardButton(f"🔗 {item.get('title', 'عضویت')}", url=item["link"])])
    rows.append([InlineKeyboardButton("✅ تأیید عضویت", callback_data=f"monshi2:check:{user_id}")])
    keyboard = InlineKeyboardMarkup(rows)
    caption = cfg.get("text") or "برای ادامه لطفاً در موارد زیر عضو شوید:"
    try:
        if cfg.get("photo_file_id"):
            await app.send_photo(user_id, cfg["photo_file_id"], caption=caption, reply_markup=keyboard)
        else:
            await app.send_message(user_id, caption, reply_markup=keyboard)
    except Exception:
        pass

@app.on_callback_query(filters.regex(r"^monshi2:check:(\d+)$"))
async def monshi2_check_callback(app, query):
    try:
        user_id = int(query.matches[0].group(1))
    except Exception:
        return await query.answer("درخواست نامعتبر است.", show_alert=True)
    if int(query.from_user.id) != user_id:
        return await query.answer("این دکمه برای کاربر دیگری است.", show_alert=True)

    cfg = _load_json_file(MONSHI2_STATE_FILE, {"enabled": False, "chats": []})
    if not cfg.get("enabled") or not cfg.get("chats"):
        return await query.answer("جوین اجباری فعال نیست.", show_alert=True)
    ...
    for item in cfg.get("chats", []):
        try:
            member = await app.get_chat_member(int(item["id"]), user_id)
            status = str(getattr(member, "status", "")).lower()
            ok = status in {"member", "administrator", "owner", "creator"} or (
                status == "restricted" and bool(getattr(member, "is_member", False))
            )
            if not ok:
                missing.append(item.get("title", "مورد"))
        except Exception:
            missing.append(item.get("title", "مورد"))

    if missing:
        return await query.answer("هنوز عضو همه موارد نشده‌اید.", show_alert=True)
...
async def mes(app, message):
    # Compare the sender ID, never the chat ID. This prevents LOVE/ENEMY
    # rules from accidentally applying to everyone in a group.
    sender_id = int(message.from_user.id) if message and message.from_user else None
    if sender_id is not None and sender_id in enemy:
        try:
            await message.reply(fosh_saz(text="."))
        except Exception as exc:
            print(f"Enemy response error: {exc}")
    elif sender_id is not None and sender_id in love:
        try:
            await message.reply(love_saz(text=choice(["❤️","💖","💝","💞","💕","💘","💗","💓"])))
        except Exception as exc:
            print(f"Love response error: {exc}")
    elif sender_id is not None and sender_id in mutey:
        try:
            await message.delete()
        except Exception:
            pass
...
# ===================== Automatic social downloader =====================
_SOCIAL_URL_RE = re.compile(
    r"https?://(?:www\.)?(?:instagram\.com|instagr\.am|tiktok\.com|"
    r"vm\.tiktok\.com|youtube\.com|youtu\.be|facebook\.com|fb\.watch|x\.com|twitter\.com)/\S+",
    re.I
)

@app.on_message(filters.me & filters.text, group=62)
async def auto_social_downloader(app, message):
    raw = message.text or ""
    if raw.startswith(".igdl") or raw.startswith(".down"):
        return
    match_url = _SOCIAL_URL_RE.search(raw)
    if not match_url:
        return
    url = match_url.group(0).rstrip(".,!?)")
    try:
        wait = await message.reply("🔗 لینک شناسایی شد\n⬇️ در حال بررسی و دانلود...")
        count = await _majid_download_media(app, message.chat.id, url, reply_to=message.id, status_message=wait)
        await wait.edit_text(f"✅ دانلود کامل شد\n📦 {count} فایل ارسال شد")
        await asyncio.sleep(1.5)
        await wait.delete()
    except Exception as exc:
        try:
            await wait.edit_text(f"❌ دانلود ناموفق بود\n`{exc}`")
        except Exception:
            pass
...
async def spam(app, message: Message):
    ...
    command = (message.command[0] or "spam").lower()

    if len(message.command) >= 2 and message.command[1].lower() in {"stop", "cancel"}:
        task = spam_tasks.pop(chat_id, None)
        if task and not task.done():
            task.cancel()
        await message.delete()
        await app.send_message(chat_id, "🛑 Spam task stopped.")
        return

    if len(message.command) >= 2 and message.command[1].lower() == "status":
        task = spam_tasks.get(chat_id)
        state = "RUNNING 🟢" if task and not task.done() else "IDLE ⚪"
        await message.edit_text(f"Spam Status: **{state}**")
        return
    ...
    try:
        ...
        if amount < 1 or amount > 10000:
            raise ValueError("تعداد باید بین 1 تا 10000 باشد.")
    except Exception as exc:
        await message.edit_text(f"❌ {exc}")
        return

    payload = " ".join(message.command[2:]).strip()
    if not payload:
        await message.edit_text("متن اسپم خالی است.")
        return
    ...
    async def runner():
        ...
        try:
            ...
            await asyncio.sleep(int(getattr(exc, "value", 5)))
        except Exception as exc:
            try:
                await app.send_message(chat_id, f"❌ Spam stopped: `{exc}`")
            except Exception:
                pass
        finally:
            if spam_tasks.get(chat_id) is asyncio.current_task():
                spam_tasks.pop(chat_id, None)

    spam_tasks[chat_id] = asyncio.create_task(runner())
...
async def delenemy(app, message: Message):
    try:
        uid = _target_user_id(app, message)
        if uid not in enemy:
            raise ValueError("این کاربر در لیست دشمنان نیست.")
        enemy.remove(uid)
        _save_feature_state()
        await message.edit_text(f"❖ {_user_label(app, uid)} از **Enemy List** حذف شد.")
    except Exception as exc:
        await message.edit_text(f"❖ {exc}")
...
async def enemylist(app, message: Message):
    if not enemy:
        return await message.edit_text("❖ Enemy List خالی است.")
    lines = [f"{i}. {_user_label(app, uid)}" for i, uid in enumerate(enemy, 1)]
...
async def setenemy(app, message: Message):
    try:
        uid = _target_user_id(app, message)
        if uid == int(admin):
            raise ValueError("نمی‌توانی خودت را در Enemy List قرار دهی.")
        if uid in enemy:
            raise ValueError("کاربر از قبل در Enemy List است.")
        enemy.append(uid)
        _save_feature_state()
        await message.edit_text(f"❖ {_user_label(app, uid)} به **Enemy List** اضافه شد.")
    except Exception as exc:
        await message.edit_text(f"❖ **SetEnemy Error:** {exc}")
...
async def dellove(app, message: Message):
    try:
        uid = _target_user_id(app, message)
        if uid not in love:
            raise ValueError("این کاربر در Love List نیست.")
        love.remove(uid)
        _save_feature_state()
        await message.edit_text(f"❖ {_user_label(app, uid)} از **LOVE List** حذف شد.")
    except Exception as exc:
        await message.edit_text(f"❖ {exc}")
...
async def lovelist(app, message: Message):
    if not love:
        return await message.edit_text("❖ LOVE List خالی است.")
    lines = [f"{i}. {_user_label(app, uid)}" for i, uid in enumerate(love, 1)]
...
async def setlove(app, message: Message):
    try:
        uid = _target_user_id(app, message)
        if uid == int(admin):
            raise ValueError("نمی‌توانی خودت را در Love List قرار دهی.")
        if uid in love:
            raise ValueError("کاربر از قبل در Love List است.")
        love.append(uid)
        _save_feature_state()
        await message.edit_text(f"❖ {_user_label(app, uid)} به **LOVE List** اضافه شد.")
    except Exception as exc:
        await message.edit_text(f"❖ **SetLove Error:** {exc}")
...
incorrect_parameters = """قفل‌های قابل استفاده:
`msg` `stickers` `gifs` `media` `games` `inline`
`url` `polls` `info` `invite` `pin`
همچنین: `all`
"""
...
async def current_chat_permissions(app, chat_id):
    ...
    for name in {
        ...
        "can_add_web_page_previews", "can_send_polls", "can_change_info",
...
async def tg_lock(app, message, permissions, perm, lock):
    if lock and perm not in permissions:
        return await message.edit_text("این قابلیت از قبل قفل است.")
    if not lock and perm in permissions:
        return await message.edit_text("این قابلیت از قبل باز است.")
    ...
    payload = {p: True for p in set(permissions)}
    try:
        await app.set_chat_permissions(message.chat.id, ChatPermissions(**payload))
    except ChatNotModified:
        return await message.edit_text("وضعیت قفل تغییری نکرد.")
    except Exception as exc:
        return await message.edit_text(f"❌ خطای Telegram:\n`{exc}`")

    await message.edit_text(f"🔒 **{perm}** {'Locked' if lock else 'Unlocked'}.")
...
async def locks_func(app, message: Message):
    try:
        ...
        parameter = message.command[1].strip().lower()
        ...
        if parameter == "all":
            if state == "lock":
                ...
                return await message.edit_text(f"🔒 همه قابلیت‌های پیام‌رسانی در **{message.chat.title or 'این چت'}** قفل شد.")
            ...
            return await message.edit_text(f"🔓 قفل همه قابلیت‌ها در **{message.chat.title or 'این چت'}** باز شد.")

        permissions = await current_chat_permissions(app, message.chat.id)
        await tg_lock(app, message, permissions, data[parameter], state == "lock")
    except Exception as exc:
        await message.edit_text(f"❌ **Lock Error:**\n`{exc}`")

@app.on_message(filters.command("locks", ".") & filters.me)
async def locks_status(app, message: Message):
    try:
        permissions = await current_chat_permissions(app, message.chat.id)
        all_permissions = set(data.values())
        locked = all_permissions - set(permissions)
        unlocked = set(permissions) & all_permissions
        await message.edit_text(
            f"🔐 **Lock Status**\n\n"
            f"🟢 Unlocked: `{len(unlocked)}`\n"
            f"🔴 Locked: `{len(locked)}`\n\n"
            + "\n".join(f"🔴 `{x}`" for x in sorted(locked))
        )
    except Exception as exc:
        await message.edit_text(f"❌ **Locks Error:**\n`{exc}`")
...
#________________________________Font______________________________
  ...
  try:
   ...
   if len(parts) < 2 or parts[1].lower() not in {"on", "off"}:
    raise ValueError("استفاده: .pvlock on/off با ریپلای، یوزرنیم یا آیدی عددی")
   target_id = _target_user_id(app, m)
   if target_id == int(admin):
    raise ValueError("نمی‌توانی قفل PV مالک سلف را فعال کنی.")
   if parts[1].lower() == "on":
    pvlock_users.add(target_id)
    _save_feature_state()
    m.edit_text(f"❋ Pv Lock برای {_user_label(app, target_id)} **ON** شد.")
   else:
    pvlock_users.discard(target_id)
    _save_feature_state()
    m.edit_text(f"❋ Pv Lock برای {_user_label(app, target_id)} **OFF** شد.")
  except Exception as er:
   m.edit_text(f"❖ **PV Lock Error:**\n`{er}`")
...
#down = app.download_media(kiri.photo.big_file_id)
  ...
  try:
   target_id = _target_user_id(app, m)
   app.block_user(target_id)
   m.edit_text(f"❖ {_user_label(app, target_id)} **Blocked**")
  except Exception as er:
   m.edit_text(f"❖ **Block Error:**\n`{er}`")

 elif text.startswith(".unblock"):
  try:
   target_id = _target_user_id(app, m)
   app.unblock_user(target_id)
   m.edit_text(f"❖ {_user_label(app, target_id)} **Unblocked**")
  except Exception as er:
   m.edit_text(f"❖ **Unblock Error:**\n`{er}`")
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  try:
   ...
   if len(parts) != 2:
    raise ValueError("استفاده: .iginfo Username")
   usernamee = _normalize_username(parts[1])
   data_ig = _majid_json("instagram/profile", {"username": usernamee}, timeout=45)
   info = _extract_instagram_profile(data_ig, usernamee)
   photo_url = _profile_photo_url(data_ig)
   verified = "✔️ Verified" if str(info["verified"]).lower() in {"true","1","yes"} else "✖️ Not Verified"
   private = "🔒 Private Account" if str(info["private"]).lower() in {"true","1","yes"} else "🌐 Public Account"
   caption = (
     "Instagram Profile\n"
     "━━━━━━━━━━━━━━━━━━\n"
     f"👤 {info['name']}\n"
     ...
     f"👤 {info['following']} Following\n"
     f"📸 {info['posts']} Posts\n"
     f"📝 Bio\n{info['bio']}\n"
     f"{verified}\n"
     f"{private}\n"
     "━━━━━━━━━━━━━━━━━━\n"
     "TiTaN_SelfSazbot @ ⤳TiTaN SelfSaz"
   )
   if photo_url:
    app.send_photo(m.chat.id, photo_url, caption=caption, reply_to_message_id=m.id)
   else:
    app.send_message(m.chat.id, caption, reply_to_message_id=m.id)
  except Exception as er:
   app.send_message(m.chat.id, f"❖ **IG Info Error:**\n`{er}`", reply_to_message_id=m.id)

 elif text.startswith(".igdl"):
  try:
   parts = text.split(maxsplit=1)
   if len(parts) != 2:
    raise ValueError("استفاده: .igdl URL")
   link = parts[1].strip()
   wait = m.reply("🔗 لینک شناسایی شد\n⬇️ در حال اتصال به MajidAPI...")
   data_dl = _majid_json(
       "instagram/download",
       {"url": link, "out": "url"},
       timeout=60
   )
   urls = _collect_urls(data_dl)
   media = [u for u in urls if _is_media_url(u)]
   urls = media or urls
   if not urls:
    raise RuntimeError("MajidAPI لینک دانلود معتبری برنگرداند.")
   for i, media_url in enumerate(list(dict.fromkeys(urls))[:10]):
    wait.edit_text(f"⬇️ در حال ارسال... {i+1}/{min(len(urls),10)}")
    if _is_media_url(media_url) and media_url.lower().split("?",1)[0].endswith((".mp4",".mov",".m4v",".webm")):
     app.send_video(m.chat.id, media_url, caption="TiTaN Self Downloader" if i == 0 else None, reply_to_message_id=m.id)
    elif _is_media_url(media_url) and media_url.lower().split("?",1)[0].endswith((".jpg",".jpeg",".png",".webp")):
     app.send_photo(m.chat.id, media_url, caption="TiTaN Self Downloader" if i == 0 else None, reply_to_message_id=m.id)
    else:
     app.send_document(m.chat.id, media_url, caption="TiTaN Self Downloader" if i == 0 else None, reply_to_message_id=m.id)
   wait.edit_text(f"✅ دانلود کامل شد\n📦 {min(len(urls),10)} فایل ارسال شد")
  except Exception as er:
   try:
    wait.edit_text(f"❌ دانلود انجام نشد\n`{er}`")
   except Exception:
    app.send_message(m.chat.id, f"❌ **Downloader Error:**\n`{er}`", reply_to_message_id=m.id)
...
#______________________________Api________________________________
  ...
  try:
   parts = text.split(maxsplit=1)
   if len(parts) != 2:
    raise ValueError("استفاده: .app نام برنامه")
   query = parts[1].strip()
   wait = m.reply("🔎 در حال جستجوی برنامه...")
   data_app = _majid_json("farsroid/search", {"s": query, "page": 1}, timeout=45)
   candidates = []
   def collect_app_items(obj):
    if isinstance(obj, dict):
     title = obj.get("title") or obj.get("name")
     link = obj.get("link") or obj.get("url")
     if title and isinstance(link, str) and link.startswith("http"):
      candidates.append((str(title), link))
     for v in obj.values():
      collect_app_items(v)
    elif isinstance(obj, list):
     for v in obj:
      collect_app_items(v)
   collect_app_items(data_app)
   candidates = list(dict.fromkeys(candidates))
   if not candidates:
    raise RuntimeError("برنامه‌ای پیدا نشد.")
   title, page_url = candidates[0]
   wait.edit_text(f"📦 {html.escape(title)}\n⬇️ در حال آماده‌سازی لینک دانلود...")
   dl_data = _majid_json("farsroid/download", {"url": page_url}, timeout=60)
   urls = [u for u in _collect_urls(dl_data) if u.startswith(("http://", "https://"))]
   if not urls:
    raise RuntimeError("لینک دانلود مستقیم پیدا نشد.")
   media_url = urls[0]
   if media_url.lower().split("?",1)[0].endswith(".apk"):
    app.send_document(m.chat.id, media_url, caption=f"📦 {title}\nTiTaN Self App Downloader", reply_to_message_id=m.id)
   else:
    app.send_document(m.chat.id, media_url, caption=f"📦 {title}\nTiTaN Self App Downloader", reply_to_message_id=m.id)
   wait.delete()
  except Exception as er:
   try:
    wait.edit_text(f"❌ **App Search Error:**\n`{er}`")
   except Exception:
    m.edit_text(f"❌ **App Search Error:**\n`{er}`")
...
#______________________________End________________________________
  ...
  try:
   ...
   cfg = _load_json_file(MONSHI2_STATE_FILE, {
       "enabled": False,
       "text": "سلام دوست عزیز، برای ادامه لطفاً در موارد زیر عضو شوید:",
       "photo_file_id": None,
       "chats": []
   })
   if action in {"on", "off"}:
    cfg["enabled"] = action == "on"
    _save_json_file(MONSHI2_STATE_FILE, cfg)
    m.edit_text(f"❋ Monshi2 **{'ON' if cfg['enabled'] else 'OFF'}** شد.")
   elif action == "text":
    if len(parts) < 3 or not parts[2].strip():
     raise ValueError("متن جدید را وارد کنید.")
    cfg["text"] = parts[2].strip()
    _save_json_file(MONSHI2_STATE_FILE, cfg)
    m.edit_text("❋ متن Monshi2 ذخیره شد.")
   elif action == "photo":
    r = m.reply_to_message
    if not r or not r.photo:
     raise ValueError("این دستور را روی یک عکس ریپلای کن.")
    cfg["photo_file_id"] = r.photo.file_id
    _save_json_file(MONSHI2_STATE_FILE, cfg)
    m.edit_text("❋ عکس Monshi2 ذخیره شد.")
   elif action in {"clearphoto", "delphoto"}:
    cfg["photo_file_id"] = None
    _save_json_file(MONSHI2_STATE_FILE, cfg)
    m.edit_text("❋ عکس Monshi2 حذف شد.")
   elif action == "add":
    if len(parts) < 3:
     raise ValueError("استفاده: .monshi2 add CHAT [INVITE_URL]")
    raw = parts[2].split()
    chat_ref = raw[0]
    invite = raw[1] if len(raw) > 1 and raw[1].startswith(("http://", "https://")) else None
    if m.reply_to_message and getattr(m.reply_to_message, "forward_from_chat", None):
     chat = m.reply_to_message.forward_from_chat
    else:
     chat = app.get_chat(chat_ref)
    chat_id = int(chat.id)
    if any(int(x.get("id")) == chat_id for x in cfg.get("chats", [])):
     raise ValueError("این چنل/گروه قبلاً ثبت شده است.")
    link = invite or (f"https://t.me/{chat.username}" if getattr(chat, "username", None) else None) or getattr(chat, "invite_link", None)
    cfg.setdefault("chats", []).append({
      "id": chat_id,
    ...
    _save_json_file(MONSHI2_STATE_FILE, cfg)
    link_state = "لینک عضویت ثبت شد" if link else "بدون لینک؛ برای مورد خصوصی Invite URL بده."
    m.edit_text(f"❋ **Monshi2 Target Added**\n{html.escape(cfg['chats'][-1]['title'])}\n{link_state}")
   elif action in {"del", "remove"}:
    if len(parts) < 3:
     raise ValueError("استفاده: .monshi2 del CHAT")
    chat = app.get_chat(parts[2].split()[0])
    chat_id = int(chat.id)
    before = len(cfg.get("chats", []))
    cfg["chats"] = [x for x in cfg.get("chats", []) if int(x.get("id")) != chat_id]
    if len(cfg["chats"]) == before:
     raise ValueError("این مورد در لیست وجود ندارد.")
    _save_json_file(MONSHI2_STATE_FILE, cfg)
    m.edit_text("❋ مورد از لیست Monshi2 حذف شد.")
   elif action == "list":
    rows = [f"{i}. {x.get('title','Unknown')} — `{x.get('id')}`" for i,x in enumerate(cfg.get("chats", []),1)]
    state = "ON 🟢" if cfg.get("enabled") else "OFF 🔴"
    m.edit_text("**MONSHI2**\nStatus: " + state + "\n" + ("\n".join(rows) if rows else "لیست خالی است."))
   else:
    state = "ON 🟢" if cfg.get("enabled") else "OFF 🔴"
    m.edit_text(
      f"**MONSHI2** — {state}\n"
      "`.monshi2 on/off`\n"
      "`.monshi2 add @channel [invite_url]`\n"
      "`.monshi2 del @channel`\n"
      "`.monshi2 text متن`\n"
      "`.monshi2 photo` روی عکس\n"
      "`.monshi2 clearphoto`\n"
      "`.monshi2 list`"
    )
  except Exception as er:
   m.edit_text(f"❖ **Monshi2 Error:**\n`{er}`")
...
try:
 ...
 # This message is the real activation proof. If it cannot be sent, do NOT
 # silently pretend the self is healthy; raise the error so bot.py reports it.
 ...
 if not startup_message_sent:
  raise RuntimeError(f"Self started but could not send startup message to Saved Messages: {last_startup_error}")

 # Mark ready only after app.start() and startup message both succeeded.
 with open("ready.flag", "w") as ready_file:
  ready_file.write("ready")
 print(Fore.GREEN + "ready.flag created; entering Pyrogram idle loop" + Fore.RESET)

 # Critical: Pyrogram update handlers need idle() to keep the client/event loop
 # alive. A plain while/sleep keeps the process alive but commands won't work.
 idle()
finally:
 try:
